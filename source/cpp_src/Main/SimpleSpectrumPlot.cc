...
bool ReadDataDirectory(std::string data_dir,  MetaDataContainer& meta_data, std::vector<double>& freq_axis, std::vector<double>& average_spectrum )
{
...
int main(int argc, char** argv)
{
    std::string usage =
    "\n"
    "Usage: SimpleSpectrumPlot <options> <directory>\n"
    "\n"
    "Plot raw spectrum from scan data in the <directory>.\n"
    "\tOptions:\n"
    "\t -h, --help               (shows this message and exits)\n"
    "\t -s, --scale              (Plot as linear scale instead of log)\n"
    ...
    static const char *optString = "hsd:";
    ...
    if(!have_data)
    {
        std::cout<<"data directory arguments is mandatory."<<std::endl;
        std::cout<<usage<<std::endl;
        return 1;
    }
    ...
    have_data =  ReadDataDirectory(data_dir, meta, freq, spectrum);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    TApplication* App = new TApplication("SimpleSpectrumPlot",&argc,argv);
    ...
    pt->SetBorderSize(1);
    pt->SetFillColor(0);
    if(meta.fExperimentName.size() != 0){  pt->AddText( std::string( std::string( "Experiment: ") + meta.fExperimentName).c_str() ); }
    if(meta.fScanName.size() != 0){pt->AddText( std::string( std::string( "Scan: ") + meta.fScanName).c_str() ); }
    if(meta.fSourceName.size() != 0){pt->AddText( std::string( std::string( "Source: ") + meta.fSourceName ).c_str() ); }
    if(meta.fRA.size() != 0){pt->AddText( std::string( std::string( "RA: ") + meta.fRA).c_str() ); }
    if(meta.fDEC.size() != 0){pt->AddText( std::string( std::string( "DEC: ") + meta.fDEC).c_str() ); }
    if(meta.fEpoch.size() != 0){pt->AddText( std::string( std::string( "Epoch: ") + meta.fEpoch).c_str() ); }
    if(meta.fStartTime.size() != 0){pt->AddText( std::string( std::string( "Start Time: ") + meta.fStartTime).c_str() ); }
    std::stringstream ss;
    ss << meta.fDuration;
    ...
    for(unsigned int j=0; j<spectrum.size(); j++)
    {
        ...
        else
        {
            ...
            {
                g->SetPoint(point_count, freq_val, 10.*TMath::Log10(val) );
                point_count++;
            }
        }
    }
    ...
    g->Draw("ALP");
    g->SetMarkerStyle(7);
    g->SetTitle("Power Spectrum" );
    g->GetXaxis()->SetTitle("Frequency (MHz)");
    
    if(use_linear_scale)
    {
        g->GetYaxis()->SetTitle("Power (arb. units.)");
    }
    else
    {
