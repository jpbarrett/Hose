...
int main(int argc, char** argv)
{
    ...
    "\t -b, --hot-load-data-dir  (path to the directory containing hot-load scan data, mandatory)\n"
    "\t -c, --cold-load-data-dir (path to the directory containing cold-load scan data, mandatory)\n"
    "\t -k, --hot-load-temp      (hot-load temperature in K, mandatory)\n"
    "\t -q, --cold-load-temp     (cold-load temperature in K, mandatory)\n"
    "\t -t, --togggle-on-off     (swap the diode state labels on <-> off)\n"
    ;

    //set defaults
    bool have_data = false;
    std::string hot_data_dir;
    std::string cold_data_dir;
    bool have_hot_data = false;
    bool have_cold_data = false;
    double hot_temp;
    double cold_temp;
    ...
    static struct option longOptions[] =
    {
        ...
        {"hot-data-dir", required_argument, 0, 'b'},
        {"cold-data-dir", required_argument, 0, 'c'},
        {"hot-load-temp", required_argument, 0, 'k'},
        {"cold-load-temp", required_argument, 0, 'q'},
        {"togggle-on-off", no_argument, 0, 't'}
    };

    static const char *optString = "hb:c:k:q:t";
    ...
    while(1)
    {
        ...
        switch(optId)
        {
            ...
            return 0;
            case('b'):
                hot_data_dir = std::string(optarg);
                have_hot_data = true;
            break;
            case('c'):
                cold_data_dir = std::string(optarg);
                have_cold_data = true;
            break;
            case('k'):
                hot_temp = std::atof(optarg);
            break;
            case('q'):
                cold_temp = std::atof(optarg);
            break;
            case('t'):
                togggle_on_off = true;
            break;
    ...
    if(!have_cold_data || !have_hot_data)
    {
    ...
    MetaDataContainer hot_meta;
    std::vector<double> hot_freq;
    std::vector<double> hot_spectrum;
    std::vector< HDataAccumulationStruct > hot_on_accumulations;
    std::vector< HDataAccumulationStruct > hot_off_accumulations;

    have_hot_data =  ReadDataDirectory(hot_data_dir, togggle_on_off, hot_meta, hot_freq, hot_spectrum, hot_on_accumulations, hot_off_accumulations);

    MetaDataContainer cold_meta;
    std::vector<double> cold_freq;
    std::vector<double> cold_spectrum;
    std::vector< HDataAccumulationStruct > cold_on_accumulations;
    std::vector< HDataAccumulationStruct > cold_off_accumulations;

    have_cold_data =  ReadDataDirectory(cold_data_dir, togggle_on_off, cold_meta, cold_freq, cold_spectrum, cold_on_accumulations, cold_off_accumulations);


    if(!have_hot_data || !have_cold_data)
    {
    ...
    if( cold_freq.size() != hot_freq.size())
    {
        std::cout<<"Error, spectrum size mismatch!"<<std::endl;
        return 1;
    }

    std::vector<double> TrxSpectrum;
    for(unsigned int i=0; i<hot_freq.size(); i++)
    {
        double hot_bin_power = hot_spectrum[i];
        double cold_bin_power = cold_spectrum[i];
        double bin_y_factor = hot_bin_power/cold_bin_power;
        double trx_bin = (hot_temp - bin_y_factor*cold_temp)/(bin_y_factor - 1.0);
        TrxSpectrum.push_back(trx_bin);
    }
    ...
    for(unsigned int j=100; j<hot_freq.size()-100; j++)
    {
        g->SetPoint(count, hot_freq[j], TrxSpectrum[j] );
        count++;
    }
