...
HSimpleMultiThreadedSpectrumDataWriterSigned::HSimpleMultiThreadedSpectrumDataWriterSigned():
    HDirectoryWriter()
    {
...
void
HSimpleMultiThreadedSpectrumDataWriterSigned::ExecuteThreadTask()
{
    //get a buffer from the buffer handler
    HLinearBuffer< spectrometer_data_s >* tail = nullptr;
...
bool
HSimpleMultiThreadedSpectrumDataWriterSigned::WorkPresent()
{
    if(this->fBufferPool->GetConsumerPoolSize(this->GetConsumerID()) == 0)
    {
        return false;
...
void
HSimpleMultiThreadedSpectrumDataWriterSigned::Idle()
{
    usleep(10);
}
